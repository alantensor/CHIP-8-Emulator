...
typedef struct
{
    ...
    uint32_t scale_factor;  // Amount to scale a CHIP8 pixel by (scales whole window)
    bool pixel_outlines;
    uint32_t insts_per_second; // CPU clock rate
    uint32_t square_wave_freq; // Frequency of square wave
    uint32_t audio_sample_rate;
    int16_t volume;
    float color_lerp_rate;       // Amount to lerp colors by, between [0.1, 1.0]
    extension_t activeExtension; // quirks/extension support (CHIP8, SUPERCHIP)
} config_t;
...
typedef struct
{
    ...
    uint8_t ram[4096];
    bool displayPixels[64 * 32];  // Emulate original CHIP8 resolution pixels
    uint32_t pixelColor[64 * 32]; // CHIP8 pixel colors to draw
...
uint32_t color_lerp(const uint32_t start_color, const uint32_t end_color, const float t)
{
    ...
    const uint8_t r = ((1 - t) * s_r) + (t * e_r);
    const uint8_t g = ((1 - t) * s_g) + (t * e_g);
    const uint8_t b = ((1 - t) * s_b) + (t * e_b);
    const uint8_t a = ((1 - t) * s_a) + (t * e_a);
...
void audio_callback(void *userdata, uint8_t *stream, int len)
{
    ...
    for (int i = 0; i < len / 2; i++)
        audio_stream[i] = ((running_sample_index++ / half_square_wave_period) % 2) ? config->volume : -config->volume;
}
...
bool initSDL(sdl_t *sdl, config_t *config)
{
    ...
    {
        SDL_Log("Init error for SDL subsystem: %s\n", SDL_GetError());
        return false;
    }
...
bool set_config_from_args(config_t *config, const int argc, char **argv)
{
    ...
    *config = (config_t){
        ...
        .activeExtension = CHIP8,   // Set default quirks/extension to plain OG CHIP-8
    };
...
bool initMachine(chip8_t *chip8, const config_t config, const char rom_name[])
{
    ...
    if (!rom)
    {
        SDL_Log("ROM %s is invalid or does not exist\n", rom_name);
        return false;
    }
    ...
    {
        SDL_Log("ROM file %s is too large. Size: %llu, Max size allowed: %llu\n",
    ...
    memset(&chip8->pixelColor[0], config.bg_color, sizeof chip8->pixelColor); // Init pixels to bg color

    return true; // Success
}

// Final cleanup
void terminateSDL(const sdl_t sdl)
{
...
void resetDisplay(const sdl_t sdl, const config_t config)
{
...
void updateScreen(const sdl_t sdl, const config_t config, chip8_t *chip8)
{
    ...
    for (uint32_t i = 0; i < sizeof chip8->displayPixels; i++)
    {
        ...
        if (chip8->displayPixels[i])
        {
            // Pixel is on, draw foreground color
            if (chip8->pixelColor[i] != config.fg_color)
            {
                // Lerp towards fg_color
                chip8->pixelColor[i] = color_lerp(chip8->pixelColor[i],
                                                  config.fg_color,
                                                  config.color_lerp_rate);
            }

            const uint8_t r = (chip8->pixelColor[i] >> 24) & 0xFF;
            const uint8_t g = (chip8->pixelColor[i] >> 16) & 0xFF;
            const uint8_t b = (chip8->pixelColor[i] >> 8) & 0xFF;
            const uint8_t a = (chip8->pixelColor[i] >> 0) & 0xFF;
        ...
        else
        {
            ...
            if (chip8->pixelColor[i] != config.bg_color)
            {
...
// 789E          asdf
// A0BF          zxcv
void handleUserInput(chip8_t *chip8, config_t *config)
{
    SDL_Event event;
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_EQUALS:
                ...
                initMachine(chip8, *config, chip8->rom_name);
                break;
...
void runInstruction(chip8_t *chip8, const config_t config)
{
    bool carry; // Save carry flag/VF value for instructions
    ...
    case 0x00:
        ...
        {
            // 0x00E0: Clear screen
            memset(&chip8->displayPixels[0], false, sizeof chip8->displayPixels);
            chip8->draw = true; // update display on next 60hz tick
        }
        else if (chip8->inst.NN == 0xEE)
        {
            // 0x00EE: Return from subroutine
            // Pop program counter off the stack, to last address on subroutine stack
    ...
    case 0x08:
        ...
        case 1:
            ...
            chip8->V[chip8->inst.X] |= chip8->V[chip8->inst.Y];
            if (config.activeExtension == CHIP8)
    ...
    case 0x0D:
    {
        ...
        for (uint8_t i = 0; i < chip8->inst.N; i++)
        {
            ...
            for (int8_t j = 7; j >= 0; j--)
            {
                ...
                bool *pixel = &chip8->displayPixels[Y_coord * config.window_width + X_coord];
...
void updateTimers(const sdl_t sdl, chip8_t *chip8)
{
...
int main(int argc, char **argv)
{
    ...
    if (!initMachine(&chip8, config, rom_name))
        exit(EXIT_FAILURE);

    resetDisplay(sdl, config);

    srand(time(NULL));

    while (chip8.state != QUIT)
    {
        handleUserInput(&chip8, &config);
